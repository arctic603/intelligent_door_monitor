...
void INIT_FUNC(void)
{
	...
	/*蓝牙设备初始化设置*/
	usart_send_str(USART3,"AT+NAMEARCTIC\r\n");
	
	delay_xms(1000);
	usart_send_str(USART3,"AT+LADDR\r\n");
	printf("蓝牙初始化正常\r\n");
	/*矩阵键盘*/
	matrix_key_init();
	key_init();
...
void Display_menu(void)
{
	...
	OLED_ShowCHinese(16,6,118);
	OLED_ShowCHinese(32,6,119);
	OLED_ShowCHinese(48,6,0);
	OLED_ShowCHinese(64,6,1);
	OLED_ShowCHinese(80,6,4);
	OLED_ShowCHinese(96,6,5);
		...
		switch(flag)
		{
			...
			case PASSWORD_UNLOCK:
				...
				if(strstr(password,num))
				{
					...
					search_unlock_record_admin(rand()%4,1);
			...
			case RFID_UNLOCK:
				...
				if(card_flag == 1 || card_flag == 2||card_flag == 3)
				{
					sg_init();
					sg_angle(0);
					sg_angle(45);
					...
					switch(card_flag)
					{
						...
						case 5:search_unlock_record_admin(5,rand()%2);break;	
					}
...
void usart_str_load(uint8_t *str_buf,uint32_t *usart_event,uint32_t *usart_cnt)
{
	...
	{
		OLED_Clear();
		OLED_DrawBMP(0,0,64,6,BLE);
		OLED_ShowCHinese(60,2,0);
		OLED_ShowCHinese(76,2,1);
