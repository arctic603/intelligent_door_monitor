...
{		
		...
		{	
			...
			if(g_ble_event)
			{
				usart_str_load(g_ble_buf,&g_ble_event,&g_ble_cnt);
			}
...
void USART1_IRQHandler(void)
{
	...
	{
		d = USART_ReceiveData(USART1);											//循环获取AT指令字符串
		...
		/*告诉CPU，已经完成接收中断的请求，可以响应新的接收中断请求*/
...
void USART3_IRQHandler(void)
{
	...
	if(USART_GetITStatus(USART3,USART_IT_RXNE) == SET)
	{
		...
		if(d == '#' || g_ble_cnt >= sizeof g_ble_buf)
		{
			g_ble_cnt = 0;
			g_ble_event = 1;	
		}else{
			g_ble_buf[g_ble_cnt++] = d;
		}
		/* 等待发送完毕 */
		while(USART_GetFlagStatus(USART1,USART_FLAG_TXE) == RESET);
